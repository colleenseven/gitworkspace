...
但是排除之后，很多人不知道怎么重新显示全部数据，其实这个操作是在筛选器里：
...
这个筛选器是通过上面的操作自动生成的，如果想撤销排除的效果，直接在筛选器里把这个“已排除”筛选删除即可。  
...
这种是向下钻取的筛选器，是因为该图表有层级，并做了向下钻取的动作：
...
如果再向下钻取，还会出现季度名称的筛选器；这种筛选器不能手动删除，后续再进行向上钻取的操作后，这些筛选器也会自动消失。

**3\. 交叉钻取筛选器**

上面的向下钻取筛选器，无法删除，但是可以编辑，可重新筛选其他项目，但是还有一种筛选器，既无法删除也无法编辑，那就是交叉筛选筛选器。

这种筛选器是向下钻取筛选器连带出来的，如果向下钻取筛选器通过交叉筛选功能传递到报表页上的另一个视觉对象，交叉钻取筛选器就会自动添加到窗格中。
